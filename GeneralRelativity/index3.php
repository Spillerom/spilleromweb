...
<script>
    ...
    var buildGrid = function() {
		...
		for( var y=0; y<gridSize; y++ ) {
			for( var x=0; x<gridSize; x++ ) {
				array.push(
					new point(
						(x * ((canvas.width * 2) / gridSize)) - (canvas.width / 2), 
						(y * ((canvas.height * 2) / gridSize)) - (canvas.height / 2)
						)
				);
			}
		}
	...
	var mainloop = function() {
		...
		// // 
		// COG(new point(
		// 	(canvas.width * 0.5) + (-Math.cos(frame * 0.02) * 180), 
		// 	(canvas.height * 0.5) + (Math.sin(frame * 0.02) * 180) ));

		// 
		COG(new point((canvas.width * 0.5), (canvas.height * 0.5)));
